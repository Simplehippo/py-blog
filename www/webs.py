...
class RequestHandler(object):
    ...
    async def __call__(self, request):
        kw = None
        if self._has_var_kw_arg or self._has_named_kw_args or self._has_var_kw_arg:
        ...
        if self._has_var_kw_arg:
            for name in self._named_kw_args:
                if name not in kw:
                    return web.HTTPBadRequest('Missing args:%s' % name)
        logging.info('call args: %s' % str(kw))
        try:
